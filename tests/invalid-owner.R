...
stopifnot(availableCores() > 2L)
className <- "MultisessionFuture"

message("- Asserting ownership ...")

message("Creating future #1:")
f1 <- future({ future:::uuid() })
stopifnot(inherits(f1, className))
...
stopifnot(inherits(f2, className))
...
f3 <- future({ f1$owner })
stopifnot(inherits(f3, className))
...
f4 <- future({ f1$owner })
stopifnot(inherits(f4, className))
...
stopifnot(inherits(f5, className))
v5 <- value(f5)
