...
#'
#'  \item{\option{future.hidden.features}:}{(logical) If `TRUE`, certain hidden features are enabled.  For example, the interception of standard output and conditions can be completely disabled by passing `stdout = NA` and `conditions = NULL` to `future()`, values that normally produces an error. (Default: `TRUE` for now, but will become `FALSE` in a later release)}
#'
#' }
#'
...
update_package_options <- function(debug = FALSE) {
  ...
  update_package_option("future.hidden.features", mode = "logical", default = TRUE, debug = debug)
}
