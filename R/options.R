...
#'
#'  \item{\option{future.resolved.timeout}:}{(numeric) The maximum time (in seconds) `resolved()` spends checking whether or not a future is resolved. If it takes longer, it will give up and return FALSE. If the option is not set, then environment variable \env{R_FUTURE_RESOLVED_TIMEOUT} will set it when the \pkg{future} package is _loaded_. (Default: `0.2` seconds)
#'  }
#'
