...
#' If random numbers are generated by the future expression, then this
#' argument must not be `FALSE` (or `NULL`) because otherwise those random
#' numbers will not be statistically sound.
#'
  ...
  if (!isTRUE(future$.rng_checked) && is.null(future$seed) && isTRUE(result$rng)) {
    ...
    if (!rng_ok) {
      ...
      if (onMisuse != "ignore") {
        label <- future$label
        if (is.null(label)) label <- "<none>"
        msg <- sprintf("UNRELIBLE RESULTS: Detected that random numbers were generated while future (%s) was resolved. Because future argument 'seed' was not set, those random numbers may not be statistical sound. To fix this, specify argument '[future.]seed', e.g. 'seed=TRUE'. To disable this check, set option 'future.rng.onMisuse' to \"ignore\".", sQuote(label))
        if (onMisuse == "error") {
          stop(msg, call. = FALSE)
        } else if (onMisuse == "warning") {
          warning(msg, call. = FALSE)
	} else {
	  stop("Unknown value on option 'future.rng.onMisuse': ",
	       sQuote(onMisuse))
	}
      }
    }
