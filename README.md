...
> b
 b.pid b1.pid b2.pid 
  7816   2276   9136 
```
which clearly show that `a` and `b` are resolved in the calling process (pid 7816) whereas the two nested futures (`b1` and `b2`) are resolved in two separate R processes (pids 2276 and 9136).
...
> b
 b.pid b1.pid b2.pid 
  9136   3908   2472 
```
First, we see that both `a` and `b` are resolved in different processes (pids 2276 and 9136) than the calling process (pid 7816).  Second, the two nested futures (`b1` and `b2`) are resolved in yet two other R processes (pids 3908 and 2472).
