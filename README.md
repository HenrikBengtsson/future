...
| `eager`         | all         |
| `lazy`          | all         | lazy evaluation - happens only if value is requested
...
| `cluster`       | all         | external R sessions on current and/or remote m| `remote`        | all         | Simple access to remote R sessions
...
> b
 b.pid b1.pid b2.pid 
  9016   8768   3888 
```
which clearly show that `a` and `b` are resolved in the calling process (pid 9016) whereas the two nested futures (`b1` and `b2`) are resolved in two separate R processes (pids 8768 and 3888).
...
> b
 b.pid b1.pid b2.pid 
  3888   5000   5824 
```
First, we see that both `a` and `b` are resolved in different processes (pids 8768 and 3888) than the calling process (pid 9016).  Second, the two nested futures (`b1` and `b2`) are resolved in yet two other R processes (pids 5000 and 5824).
