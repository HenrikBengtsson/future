...
* When a future is constructed, _global variables are identified_.  If there are futures among the globals, these are all resolved before continue.  For asynchronous evaluation, globals are also exported to the R process/session that will be evaluating the future expression.  For lazy futures, the globals are also "frozen" (cloned to a local environment of the future).  Also, in order to protect against exporting too large objects by mistake, there is a built-in assertion that the total size of all globals is less than a given threshold (controllable via an option, cf. `help("future.options")`).  If the threshold is exceeded, an informative error is thrown.
...
> b
 b.pid b1.pid b2.pid 
  7177   7246   7247 
```
which clearly show that `a` and `b` are resolved in the calling process (pid 7177) whereas the two nested futures (`b1` and `b2`) are resolved in two separate R processes (pids 7246 and 7247).
...
> b
 b.pid b1.pid b2.pid 
  7249   7250   7251 
```
First, we see that both `a` and `b` are resolved in different processes (pids 7248 and 7249) than the calling process (pid 7177).  Second, the two nested futures (`b1` and `b2`) are resolved in yet two other R processes (pids 7250 and 7251).
...
## Globals
Whenever an R expression is to be evaluated asynchronously (in parallel) or via lazy evaluation, global objects have to be identified and passed to the evaluator.  They need to be passed exactly as they were at the time the future was created, because, for a lazy future, globals may otherwise change between when it is created and when it is resolved.  For asynchronous processing, the reason globals need to be identified is so that they can be exported to the process that evaluates the future.  Furthermore, if there are futures among the globals, they have to be resolved before the future (that depends on them) is created.
