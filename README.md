...
> b
 b.pid b1.pid b2.pid 
  8080   1268   6524 
```
which clearly show that `a` and `b` are resolved in the calling process (pid 8080) whereas the two nested futures (`b1` and `b2`) are resolved in two separate R processes (pids 1268 and 6524).
...
> b
 b.pid b1.pid b2.pid 
  6524   1968   4308 
```
First, we see that both `a` and `b` are resolved in different processes (pids 1268 and 6524) than the calling process (pid 8080).  Second, the two nested futures (`b1` and `b2`) are resolved in yet two other R processes (pids 1968 and 4308).
