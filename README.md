...
### A first example
Here is an example illustrating how the basics of futures work.  First, consider the following two R expressions using plain R code:
```r
> v <- {
+   message("Resolving...")
+   3.14
...
[1] 3.14
```
It works by assigning the value of an expression to variable `v` and we then print the value of `v`.  Moreover, when the expression for `v` is evaluated we also print a message.  Here is the analog construct using futures
```r
> library(future)
> v %<-% {
+   message("Resolving...")
...
[1] 3.14
```
Note how the future is resolved as soon as we create it using `%<-%`.  This is because the default strategy for resolving futures is to evaluate them in an "eager" and synchronous manner closely immitating how R itself does it (using either the implicit or the explicit evaluation strategy).
...
Futures can be created either _implicitly_ or _explicitly_.  In the above introductory example we used implicit futures created via the `x %<-% { expr }` construct.  We could have also created explicit futures using the `f <- future({ expr })` and `x <- value(f)` constructs.  More precisely, we could achieve the same results using:
```r
> library(future)
> plan(lazy)
> f <- future({
...
### The Future API
For explicit futures, there are in principal only three methods:

* `f <- future({ expr })` - creates a future
* `v <- value(f)` - gets the value of the future (blocks if not yet resolved)
* `resolved(f)` - checks whether the future is resolved or not

For implicit futures, there are three methods:
* `v %<-% { expr }` - creates promise to a future value
* `f <- futureOf(v)` - gets the future itself
* `resolved(f)` - checks whether the future is resolved or not
...
To install the pre-release version that is available in branch `feature/topology`, use:
```r
source('http://callr.org/install#HenrikBengtsson/future@feature/topology')
```
