...
> b
  b.pid b.1.pid b.2.pid 
   8016    6264    7748 
```
which clearly show that `a` and `b` are resolved in the calling process (pid 8016) whereas the two nested futures (`b.1` and `b.2`) are resolved in two separate R processes (pids 6264 and 7748).
...
> b
  b.pid b.1.pid b.2.pid 
   7748    4852    4684 
```
First, we see that both `a` and `b` are resolved in different processes (pids 6264 and 7748) than the calling process (pid 8016).  Second, the two nested futures (`b.1` and `b.2`) are resolved in yet two other R processes (pids 4852 and 4684).
