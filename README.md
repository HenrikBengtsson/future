...
> b
 ...
 24830  24924  24925 
```
which clearly show that `a` and `b` are resolved in the calling process (pid 24830) whereas the two nested futures (`b1` and `b2`) are resolved in two separate R processes (pids 24924 and 24925).
...
> b
 ...
 24927  24930  24932 
```
First, we see that both `a` and `b` are resolved in different processes (pids 24926 and 24927) than the calling process (pid 24830).  Second, the two nested futures (`b1` and `b2`) are resolved in yet two other R processes (pids 24930 and 24932).
...
To install the pre-release version that is available in Git branch `feature/dep-lazy` on GitHub, use:
```r
source('http://callr.org/install#HenrikBengtsson/future@feature/dep-lazy')
```
