...
[1] 3.14
```
With asynchronous futures the current/main R process does _not_ block, which me
ns it is available for further processing while the futures are being resolved
...
> b
  b.pid b.1.pid b.2.pid 
   8156    2068    3440 
```
which clearly show that `a` and `b` are resolved in the calling process (pid 8156) whereas the two nested futures (`b.1` and `b.2`) are resolved in two separate R processes (pids 2068 and 3440).
...
> b
  b.pid b.1.pid b.2.pid 
   3440    5428    1900 
```
First, we see that both `a` and `b` are resolved in different processes (pids 2068 and 3440) than the calling process (pid 8156).  Second, the two nested futures (`b.1` and `b.2`) are resolved in yet two other R processes (pids 5428 and 1900).
