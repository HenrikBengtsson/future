...
> b
 b.pid b1.pid b2.pid 
  9056   6644   6400 
```
which clearly show that `a` and `b` are resolved in the calling process (pid 9056) whereas the two nested futures (`b1` and `b2`) are resolved in two separate R processes (pids 6644 and 6400).
...
> b
 b.pid b1.pid b2.pid 
  6400   6344   8336 
```
First, we see that both `a` and `b` are resolved in different processes (pids 6644 and 6400) than the calling process (pid 9056).  Second, the two nested futures (`b1` and `b2`) are resolved in yet two other R processes (pids 6344 and 8336).
