...
> b
 b.pid b1.pid b2.pid 
  8884  10184   6300 
```
which clearly show that `a` and `b` are resolved in the calling process (pid 8884) whereas the two nested futures (`b1` and `b2`) are resolved in two separate R processes (pids 10184 and 6300).
...
> b
 b.pid b1.pid b2.pid 
  6300   9344   9492 
```
First, we see that both `a` and `b` are resolved in different processes (pids 10184 and 6300) than the calling process (pid 8884).  Second, the two nested futures (`b1` and `b2`) are resolved in yet two other R processes (pids 9344 and 9492).
