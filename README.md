...
> b
 b.pid b1.pid b2.pid 
  9284   3972   9872 
```
which clearly show that `a` and `b` are resolved in the calling process (pid 9284) whereas the two nested futures (`b1` and `b2`) are resolved in two separate R processes (pids 3972 and 9872).
...
> b
 b.pid b1.pid b2.pid 
  9872   1232   6416 
```
First, we see that both `a` and `b` are resolved in different processes (pids 3972 and 9872) than the calling process (pid 9284).  Second, the two nested futures (`b1` and `b2`) are resolved in yet two other R processes (pids 1232 and 6416).
