...
> b
 ...
 10864   9548   5004 
```
which clearly show that `a` and `b` are resolved in the calling process (pid 10864) whereas the two nested futures (`b1` and `b2`) are resolved in two separate R processes (pids 9548 and 5004).
...
> b
 b.pid b1.pid b2.pid 
  5004  10540   2664 
```
First, we see that both `a` and `b` are resolved in different processes (pids 9548 and 5004) than the calling process (pid 10864).  Second, the two nested futures (`b1` and `b2`) are resolved in yet two other R processes (pids 10540 and 2664).
