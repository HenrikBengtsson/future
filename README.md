...
> b
 b.pid b1.pid b2.pid 
  5390   5456   5457 
```
which clearly show that `a` and `b` are resolved in the calling process (pid 5390) whereas the two nested futures (`b1` and `b2`) are resolved in two separate R processes (pids 5456 and 5457).
...
> b
 b.pid b1.pid b2.pid 
  5459   5460   5461 
```
First, we see that both `a` and `b` are resolved in different processes (pids 5458 and 5459) than the calling process (pid 5390).  Second, the two nested futures (`b1` and `b2`) are resolved in yet two other R processes (pids 5460 and 5461).
