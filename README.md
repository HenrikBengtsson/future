...
> b
 b.pid b1.pid b2.pid 
  7960   8063   8064 
```
which clearly show that `a` and `b` are resolved in the calling process (pid 7960) whereas the two nested futures (`b1` and `b2`) are resolved in two separate R processes (pids 8063 and 8064).
...
> b
 b.pid b1.pid b2.pid 
  8066   8067   8068 
```
First, we see that both `a` and `b` are resolved in different processes (pids 8065 and 8066) than the calling process (pid 7960).  Second, the two nested futures (`b1` and `b2`) are resolved in yet two other R processes (pids 8067 and 8068).
