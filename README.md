...
> b
 b.pid b1.pid b2.pid 
  9592   9682   9683 
```
which clearly show that `a` and `b` are resolved in the calling process (pid 9592) whereas the two nested futures (`b1` and `b2`) are resolved in two separate R processes (pids 9682 and 9683).
...
> b
 b.pid b1.pid b2.pid 
  9685   9686   9687 
```
First, we see that both `a` and `b` are resolved in different processes (pids 9684 and 9685) than the calling process (pid 9592).  Second, the two nested futures (`b1` and `b2`) are resolved in yet two other R processes (pids 9686 and 9687).
